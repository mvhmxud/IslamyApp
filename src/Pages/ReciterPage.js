...
import ListOfSurah from "../components/Layout/ListOfSurah" 
import { useEffect, useState } from "react";
import { useDispatch, useSelector } from "react-redux";
import { LoadingSliceActions } from "../store/LoadingSlice";
import { PlayerSliceActions } from "../store/PlayerStore";
import { SurahName } from "../Hooks & Functions/SurahName";

function ReciterPage(props) {
    const params = useParams()
    const history = useHistory()
    const dispatch =  useDispatch()
    const currentSurah = useSelector((state)=>state.PlayerSlice.currentSurah.src)
    useEffect(()=>{
        if(params.id > 12){ ///12 is reciters number from the api to prevent unwanted routing
            history.push('/error')
        }
    },[])
    ...
    const clickHandler=(key)=>{
        ...
        axios.get(`https://api.quran.com/api/v4/chapter_recitations/${params.id}/${key}`).then((res)=>
        dispatch(PlayerSliceActions.SetCurrentSurah({name :SurahName(res.data.audio_file.chapter_id),src:res.data.audio_file.audio_url}))
...
const [surahList ,setSurahList ] = useState(list)
const searchHandler = (value) => {
    if(value !==' '){
        if(isArabic){
            const result = list.filter((surah) => surah.name.toLocaleLowerCase().includes(value.toLocaleLowerCase()))
            setSurahList(result)
        }else{
            const result = list.filter((surah) => surah.transliteration.toLocaleLowerCase().includes(value.toLocaleLowerCase()))
            setSurahList(result)
        }
    }else{
        setSurahList(list)
...
return (<div className=" bg-background dark:bg-darkmodeDark">
        ...
        <Search Placeholder={isArabic  ? 'ادخل اسم السورة' : 'Enter surah name '} handler={searchHandler}/>
        <ListOfSurah header={isArabic ? 'القرآن الكريم' : 'List of Chapters (Surah)'}  list={surahList} handler={clickHandler}/>
        <Footer/>
    </div>);
}
